...
|---|---|---|---|
| Сбор объявлений | Парсеры Encar, Goonet, китайских площадок каждые несколько часов | R0–R1: ленты нет, варианты под запрос даёт экспортёр-партнёр. R2: лицензированный фид | Encar запрещает автоматический сбор, копирование существенной части чужой базы нарушает ст. 1334 ГК (`00-context-audit.md`, И9) |
...
Чем жертвуем: Bronze не масштабируется — при > 12 часах оператора на сделку маржа исчезает (`unit-economics.md`, «в»). Это и есть сигнал переходить на Silver.
...
C4Context
    ...
    Rel(bank, payagent, "Раскрывает по инвойсу и коносаменту")
    Rel(payagent, exporter, "Оплата выкупа")
...
| **Экспортёр в Японии** | Варианты, торги, выкуп, отгрузка, документы на клиента | Договор с 1 партнёром; связь через оператора | Второй экспортёр | Документы на имя клиента [ДОПУЩЕНИЕ] |
...
| Источник данных о машинах | R0: не нужен; R1: варианты от экспортёра; R2: лицензированный фид | Парсинг запрещён правилами Encar и рискует ст. 1334 ГК (И9) | Парсинг — только как осознанный риск, не рекомендуем; партнёрская программа площадки — искать |
...
|---|---|---|
| Данные площадок без разрешения (И9) | Блокировка, иск по ст. 1334 ГК, запрет Encar | Нет ленты в MVP; в R2 — фид по договору или партнёрская программа; парсинг не используем |
...
| Утечка или зарубежное хранение паспортных данных | Штраф до 18 млн ₽ | Сервисы с хранением в РФ, паспорта не в общем чате, минимум доступа |
| Bronze не масштабируется | Часы оператора > 12 на сделку — маржа ноль | Порог перехода на Silver — метрика MVP (`hypotheses.md`, раздел 7) |

---

## Допущения и открытые вопросы

**Уровень неопределённости:** критические разделы — акторы, внешние системы, контейнеры, связи — заполнены (0/4); вспомогательные — потоки, решения, риски, персональные данные — заполнены (0/4) → **0,00** по заполненности. Содержательная неопределённость — ниже.
...
- Открыто: какой банк подтвердит аккредитив (Г-11); кто экспортёр и брокер; права на показ данных фида клиентам; нужна ли регистрация оператора ПДн до R1; касса по 54-ФЗ (`unit-economics.md`, «ж»).
